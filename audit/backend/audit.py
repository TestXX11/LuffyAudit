...
class AuditLogHandler(object):
    ...
    def parse(self):
        ...
        return cmd_list
def file_traversal():
    file_result=subprocess.Popen('ls ../../log/',shell=True,stdout=subprocess.PIPE,stderr=subprocess.PIPE)
    print(file_result.stdout.read().decode('utf-8'))

if __name__ == "__main__":

    file_traversal()
    file_id=input('input file id>>>>: ')
    parser = AuditLogHandler('../../log/ssh_audit_{file_id}.log'.format(file_id=file_id))
